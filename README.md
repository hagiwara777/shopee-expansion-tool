...
ASIN Resolver Tool Ver0.3の既存手動フローは、商品名からAmazon商品をアプリ内部で検索する機能ではなく、外部AIの返答から候補を抽出する補助ツールです。CSVファイルのアップロード、Expansion Toolへの自動投入、Shopee API連携、自動出品、Amazonページ操作、ブラウザ自動操作、Gemini APIの自動呼び出しは行いません。Guardrail FilterもResolverからは呼び出しません。source_idの永続保存も行いません。Ver0.5aのOpenAI API利用は、下記の独立検証タブに限定されます。

## ASIN Resolver Tool Ver0.5a

Ver0.5aは、`OpenAI検索テスト`タブでOpenAI Responses APIとWeb searchがAmazon.co.jp候補検索に使えるかを独立して確認するための技術検証機能です。1行1商品、最大5件を`T0001`形式の検証専用source_idで処理し、元の`input_title`と既存の検索用タイトル生成結果を表示します。既存Resolverの`R0001`形式source_id、候補表、再検索、CSVへは自動連結しません。

APIは`OpenAI Web検索を実行`ボタンを押したときだけ呼ばれます。プレビュー、入力編集、タブ切替、Streamlitの再描画ではOpenAI APIもKeepa APIも呼びません。Web searchは一般Webを日本向け位置情報とmediumの検索コンテキストで利用し、英語タイトルに対して日本語名・カタカナ表記・公式情報も考慮します。返却URLはアプリ側でHTTPS、Amazon.co.jpドメイン、`/dp/ASIN`または`/gp/product/ASIN`、10桁英数字ASINの条件で再検証するため、候補として採用されるのはAmazon.co.jp URLだけです。

表示されるURLはOpenAI返却候補であり、`URL_FORMAT_VALIDATED`はURL形式だけを確認した状態です。`KEEPA_NOT_CHECKED`および`EXISTENCE_NOT_VERIFIED`のとおり、Keepa実在確認・Amazon実ページ確認は行いません。実行結果では、Web searchが返したsourceのURL、タイトル、URL由来domainだけをこのブラウザセッション内に表示します。source本文、OpenAIレスポンス全文、APIキー、認証情報は保存・表示しません。Expansion Tool、Guardrail、Product Finder、Shopee処理、既存Resolver候補、結果CSVへの自動反映も行いません。
...
`OPENAI_API_KEY`はVer0.5aの`OpenAI検索テスト`を明示実行するときだけ必要です。OpenAI APIの利用料金は別途発生する可能性があります。APIキー、Authorizationヘッダー、レスポンス全文は画面・ログ・Gitへ出力しません。
