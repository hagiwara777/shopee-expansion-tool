...
def set_guardrails(monkeypatch, statuses: list[str] | tuple[str, ...], *, mutate=None):
    ...
    def fake_apply_guardrails(rows):
        inputs = list(rows)
...
def evaluate(rows, inventories, *, source_type=EXPANSION_SOURCE_TYPE, expected_shop_count=None):
...
def test_guardrail_dictionary_error_stops_whole_evaluation(monkeypatch):
    def raise_dictionary_error(rows):
...
@pytest.mark.parametrize("marketplace", ["PH", "MY", "TH", "", None])
def test_only_sg_marketplace_is_allowed(monkeypatch, marketplace):
    set_guardrails(monkeypatch, ["SAFE"])
