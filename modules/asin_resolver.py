...
def build_ai_prompt(product_names_text: str) -> str:
    ...
    return (
        "このメッセージをそのまま実行してください。あなたの返答はASIN Resolverの「AI返答」欄へそのまま貼り付けて使用します。\n"
        "追加質問、説明、前置き、後書き、Markdown表、コードフェンスは付けず、TSV本文だけを返してください。\n\n"
        "以下の商品について、Amazon.co.jpの商品URLを検索してTSV形式で返してください。\n"
        "1行目は source_id<TAB>input_title<TAB>amazon_url としてください。\n"
        "各行のsource_idとinput_titleは入力行どおりに保持してください。1つの商品に複数候補がある場合は、同じsource_idで複数行を返してください。\n"
        "入力されたすべてのsource_idについて、必ず1行以上を商品順のまま返してください。\n"
        "Amazon.co.jpの商品URLだけを返し、amazon.com、amazon.sgなどAmazon.co.jp以外のURLは返さないでください。\n"
        "見つからない、または検索できない場合は追加質問をせず「不明」としてください。URLが不明な場合は推測URLを作らないでください。\n"
        "返答本文以外の説明は付けないでください。\n\n"
        "出力形式:\n"
...
def build_retry_prompt(rows: Iterable[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "各行のsource_idとinput_titleは入力行どおりに保持してください。1つの商品に複数候補がある場合は、同じsource_idで複数行を返してください。\n"
        "入力されたすべてのsource_idについて、必ず1行以上を商品順のまま返してください。\n"
        "Amazon.co.jp以外のURLは返さないでください。見つからない、または検索できない場合は追加質問をせず「不明」としてください。\n"
        "推測URLは作らず、返答本文以外の説明は付けないでください。1候補を必ず1行にし、各列は実際のタブ文字で区切ってください。\n\n"
        "出力形式:\n"
        "source_id\tinput_title\tamazon_url\n\n"
        "商品名:\n"
        f"{'\n'.join(source_lines)}"
