...
- Category Mapper（Safety判定を通過した候補について、対象市場ごとのCategory IDを決定・確認する。PHは正式成果あり、SGはCategory Mapper Minimum Betaの実装候補を検証中）
...
3. SGはoperation INACTIVE / development ALLOWEDとし、SG Category Mapper Minimum Betaだけを開発する。実装候補はOwner Acceptanceとformal main統合まで正式成果と扱わず、`sg.safety.baseline`はprotected capabilityとして維持する。
