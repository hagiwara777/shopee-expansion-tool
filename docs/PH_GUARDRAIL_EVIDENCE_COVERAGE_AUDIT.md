...
実データでPHのGateを試す前に、`PH Guardrail Block Register v1` を作る。これは全規約の対応表でも、ルールを一気に増やす作業でもない。現在の`BLOCK`候補だけについて、次を一つの表にする。

1. ルールID・対象語・`BLOCK`となる理由
2. Shopee公式ポリシーまたは自社ペナルティ事例という根拠と、確認日
3. Gateで`EXCLUDE`になることを確認するテスト
4. 実際の出品結果で新しい一発アウト事例が判明した場合の追記欄

`REVIEW`は、完全にアウトとは言えない候補を人が試すか判断するために残す。ブランド・知的財産、許認可、公式ポリシーの未対応分類を、根拠なしに一律`BLOCK`へ広げない。ルール追加、実データテスト、Shopeeへの書込みは別途承認が必要である。
