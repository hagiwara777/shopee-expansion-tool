...
これは設計ゲート用の提案である。オーナーが実際に役立つ完成像と一致することを
確認するまで、完成条件・範囲・優先順位の正式決定ではない。
...
Expansionの画面には現在SG辞書による一次判定が残る。これは候補生成と市場別判断が
混ざっている現行差分であり、PHを含む市場別の正式判断は出品前保安ゲートへ寄せられるかを
次の差分監査で確認する。Expansion機能を削除したり、候補生成の中核から外したりしない。
...
1. Resolver、Expansion、Gate、Category Mapperについて、この役割分担と現行実装との差分を
   読み取り専用で監査する。
2. 必要な最小修正とテストを提案する。
3. オーナーの明示承認後だけ、少量の実データ、必要な外部API、実画面で最初の市場の
   受入を行う。
