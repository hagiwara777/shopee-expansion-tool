...
`prelisting_gate/v1` はSG、1ショップ（`SG_E2E_SHOP_1`）、候補4件のローカルfixtureです。`prelisting_gate/v2` はPH、空inventory、1ショップ（`PH_SHOP_1`）、候補3件のローカルfixtureです。どちらも外部APIを呼びません。
...
このフォルダには、選択したfixture versionに対応するCSV 2件だけが存在します。

- `01_candidates_EXPECT_<candidate_count>.csv`
- `02_existing_<marketplace>_<shop_label>_EXPECT_<existing_listing_row_count>.csv`

既定のv1では `01_candidates_EXPECT_4.csv` と `02_existing_SG_SHOP_1_EXPECT_1.csv` が生成されます。PHのv2を準備する場合は次を実行します。

```powershell
powershell -ExecutionPolicy Bypass -File scripts\e2e\prepare_browser_e2e.ps1 -Suite prelisting_gate -FixtureVersion v2
```

v2では `01_candidates_EXPECT_3.csv` と `02_existing_PH_PH_SHOP_1_EXPECT_0.csv` が生成されます。PH空inventoryは5行目の16列ヘッダーだけを持つ正式な空CSVです。
...
1. `READY.txt` を開き、suite、fixture version、marketplace、shop_label、候補件数が選択したfixtureと一致することを確認します。v1はSG／`SG_E2E_SHOP_1`／4件、v2はPH／`PH_SHOP_1`／3件です。
...
フォルダ指定では、expected.jsonのaudit行数に一致する監査用CSVを一意に特定できる必要があります。検証はUTF-8 BOM、固定列と列順、ファイル名、行数、marketplace、ELIGIBLE／REVIEW／EXCLUDE件数、ASIN重複・混入、入力fixtureとの対応、ASINごとの最終判定・Guardrail判定・reason_codes・既出品判定を確認し、差分付きで `PASS` または `FAIL` を出します。

## fixtureを更新するとき

version済みfixtureは上書きしません。入力形式・期待値・判定根拠を変更する必要がある場合は次のversionディレクトリを新設し、`prepare_browser_e2e.ps1 -FixtureVersion <version>` で選択可能にして、対応する契約テストを更新・追加します。外部APIを使うE2Eは別suiteに分け、明示承認がある場合だけ実行します。
