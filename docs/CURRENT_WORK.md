...
オーナーが、画面で「候補を集める→市場別Gate→Category / Brand準備」の流れが分かるかを確認する。問題がなければ、CodexがGPT独立レビュー依頼文とpush／Draft PR判断用の平易な報告を作成する。
