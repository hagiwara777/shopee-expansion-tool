...
オーナーが画面で、ASIN Resolverの通常画面に内部Evidence Batchが表示されず、「候補生成 → 市場別Gate → Category／Brand準備」の導線が迷わず分かるかを再確認する。問題なければ、Codexがpush・Draft PR作成の平易な報告を行う。
